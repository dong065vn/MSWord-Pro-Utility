...
Func _CreateTabQuickUtils()
    ...
    $g_btnShowDocInfo = _CreateButton("Thong tin chi tiet", 40 + ($iColW + $iGap) * 2, $iY + 100, 210, 28, 0x3498DB, 9, 600)
    ...
    $g_btnUnlinkFields = _CreateButton("Unlink All Fields", 40 + ($iColW + $iGap) * 2, $iY + 332, 210, 24, 0xE74C3C)
    ...
    $g_inputHeadingPrefixFix = _CreateInput("", 88, $iY + 370, 100, 24)
    GUICtrlSetTip(-1, "Loc theo tien to de muc. Chap nhan: 1 | 1.2 | A. | A.2. | I.1. De trong de sua toan bo.")
    _CreateLabel("Thay thanh:", 198, $iY + 374, 62, 20)
    $g_inputHeadingPrefixReplace = _CreateInput("", 265, $iY + 370, 85, 24)
    GUICtrlSetTip(-1, "Neu co gia tri, se thay tien to da loc bang tien to moi. Vi du: 1. -> A. | 1.2. -> B.2.")
    _CreateLabel("Pham vi:", 360, $iY + 374, 52, 20)
    $g_cboHeadingFixScope = GUICtrlCreateCombo("", 415, $iY + 370, 90, 24, $CBS_DROPDOWNLIST)
    GUICtrlSetData($g_cboHeadingFixScope, "Toan document|Vung chon", "Toan document")
    GUICtrlSetTip(-1, "Chon pham vi quet va thay tien to de muc")
    _CreateLabel("Ngat sau so:", 40, $iY + 401, 70, 20)
    $g_inputHeadingSeparatorFix = _CreateInput(" ", 115, $iY + 397, 70, 24)
    GUICtrlSetTip(-1, "Nhap dau ngan cach muon dung sau so: space, . , : , - , tab, none. Vi du ''. '' -> 2.4. Tieu de")
    $g_btnFixHeadingNumberDots = _CreateButton("Quet && thay tien to", 520, $iY + 367, 155, 28, 0x16A085)
    GUICtrlSetTip(-1, "Chuan hoa de muc va co the thay tien to hang loat. Vi du: 1.2 -> A.2 | 1.2.1 -> A.2.1")
    GUICtrlCreateLabel("Vi du: 1. 2 -> 1.2 | 2.4. -> 2.4 | separator ''. '' -> 2.4. Tieu de | separator '': '' -> 2.4: Tieu de", _
        198, $iY + 401, 522, 16)
    GUICtrlCreateLabel("Loc prefix: 1. -> quet 1.2 / 1.2.1 | Thay thanh: A. -> A.2 / A.2.1 | Alias separator: tab, space, none", _
        40, $iY + 419, 680, 16)
    _EndGroup()
